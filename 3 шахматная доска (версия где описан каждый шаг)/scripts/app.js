let app = {
    ...
    generateBoard() {
        ...
        for (let i = 0; i <this.config.rows.length ; i++) {
            ...
            //let row = "";
            board += this.generateRow(rowStartWithColor, this.config.rows[i]);
    ...
    generateRow(startWithColor, rowNum) {
        // 2. создаем переменную urrentColorClass, в которую пишем startWithColor
        ...
        for (let i = 0; i <this.config.cols.length ; i++) {
            ...
            //let field='';
            row += this.generateField(currentColorClass, rowNum, this.config.cols[i]);
            ...
            /*if (currentColorClass=="white") {
                ...
                //field= this.generateField(, , this.config.cols[i]);
    ...
    insertRowsNumbers() {
        ...
        for (let i = 0; i <this.config.rows.length ; i++) {
            ...
            trs[i].insertAdjacentHTML("afterbegin", `<td>${this.config.rows[i]}</td>`);
        }
    },
